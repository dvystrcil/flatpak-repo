...
This repository hosts Flatpak packages for MekHQ and other applications.
...
2. Add source files (wrapper script, configs, etc.) to the same directory
3. Commit and push - CI will build and publish to GitHub Pages

## CI/CD

- **main branch push**: Builds all Flatpaks and publishes to GitHub Pages
- **pull request**: Validates build succeeds without publishing
...
flatpack-repo/
├── README.md
├── com.megamek.mekhq.json      # MekHQ Flatpak manifest
├── mekhq-wrapper.sh            # Wrapper script to launch MekHQ
└── .github/
    └── workflows/
        ├── build.yml           # Main CI: builds & publishes to GitHub Pages
        ├── pr-validation.yml   # PR-only validation
